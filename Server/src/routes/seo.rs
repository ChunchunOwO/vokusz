...
fn oembed_link(base: &str, page_url: &str) -> String {
    format!(
        r#"    <link rel="alternate" type="application/json+oembed" href="{base}/oembed?url={enc}&amp;format=json" title="vokusz oEmbed">
"#,
        base = base,
...
/// Builds an HTML landing page for human visitors that attempts a `vokusz://`
/// protocol redirect, then falls back to the web client hash URL.
///
/// - `vokusz_uri`: the `vokusz://connect/...` URI to attempt
      ...
      setTimeout(function() {{
        if (!opened) {{
          document.getElementById("status").innerHTML =
            'Vokusz not detected &mdash; <a href="/{web_fragment}">continue in browser</a>';
        }}
      }}, 2000);
    </script>
  </body>
...
pub async fn channel_snapshot(
    ...
    // The vokusz client handles registration/invite prompts as needed.
    if !is_crawler(&headers) {
        let host = extract_host(&headers);
        let vokusz_uri = format!(
            "vokusz://connect/{}/{}",
            escape_html(&host),
...
{body_html}    </main>
    <footer>
      <p><a href="/{fragment_url}">Open in Vokusz</a></p>
    </footer>
  </body>
</html>"#,
        title = title,
...
{channel_list_html}        </ul>
    ...
    <footer>
      <p><a href="/#{space_slug_escaped}">Open in Vokusz</a></p>
    </footer>
  </body>
</html>"#,
        title = title,
...
pub async fn oembed(
    ...
    let path = query
        ...
        .find("/s/")
        .map(|i| &query.url[i..])
        .ok_or_else(|| AppError::BadRequest("url is not a vokusz resource".to_string()))?;
    ...
    if segs.first().map(String::as_str) != Some("s") || segs.len() < 2 {
        return Err(AppError::BadRequest(
            "url is not a vokusz resource".to_string(),
        ));
    }
