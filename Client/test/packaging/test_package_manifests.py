...
class PackageManifestsTest(unittest.TestCase):
    ...
    def test_generates_all_managers_with_matching_version_and_no_placeholders(self):
        ...
        self.assertIn('winget/manifests/v/Vokusz/Vokusz/1.2.3/Vokusz.Vokusz.installer.yaml', files)
        self.assertIn('scoop/vokusz.json', files)
        self.assertIn('chocolatey/tools/chocolateyInstall.ps1', files)
        self.assertIn('homebrew/Casks/vokusz.rb', files)
        self.assertIn('flatpak/io.github.ChunchunOwO.vokusz.json', files)
    ...
    def test_rejects_wrong_release_or_external_asset_urls(self):
        for url in ['https://evil.test/installer.exe', f'{PACKAGING.REPOSITORY}/releases/download/v9.9.9/vokusz-windows-x86_64-setup.exe']:
    ...
    def test_rendering_is_repeatable_and_check_never_mutates(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            target = output / 'scoop/vokusz.json'
    ...
    def test_updater_hooks_do_not_relocate_or_delete_personal_data(self):
        files = PACKAGING.rendered_files(PACKAGING.release_values(self.release, self.sums))
        scoop = json.loads(files['scoop/vokusz.json'])
        self.assertNotIn('persist', scoop)
        self.assertEqual(scoop['bin'], 'vokusz.exe')
        self.assertIn('vokusz.package-manager', scoop['post_install'])
        self.assertIn(b'/PACKAGE_MANAGER=chocolatey', files['chocolatey/tools/chocolateyInstall.ps1'])
        cask = files['homebrew/Casks/vokusz.rb'].decode()
        self.assertNotIn('postflight', cask)
        self.assertIn('zap trash: "~/Library/Caches/com.vokusz.app"', cask)
        self.assertIn(b'export VOKUSZ_PACKAGE_MANAGER=flatpak', files['flatpak/vokusz'])

    def test_flatpak_ids_dependencies_and_sandbox_are_consistent(self):
        files = PACKAGING.rendered_files(PACKAGING.release_values(self.release, self.sums))
        manifest = json.loads(files['flatpak/io.github.ChunchunOwO.vokusz.json'])
        self.assertEqual(manifest['app-id'], 'io.github.ChunchunOwO.vokusz')
        ...
        metadata = ET.fromstring(files['flatpak/io.github.ChunchunOwO.vokusz.metainfo.xml'])
