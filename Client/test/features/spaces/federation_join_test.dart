...
void main() {
  ...
  group('federate deep link', () {
    test('parses vokusz://federate/<spaceId>@<domain>', () {
      final parsed = ServerUri.parseDeepLink('vokusz://federate/42@b.example');
    ...
    test('rejects a federate link without a domain', () {
      expect(ServerUri.parseDeepLink('vokusz://federate/42'), isNull);
    });

    test('preserves qualified spaceId in federate deep link', () {
      final parsed = ServerUri.parseDeepLink(
          'vokusz://federate/42@olddomain.example@newdomain.example');
      expect(parsed?.route, 'federate');
