...
void main() {
  ...
  test('partial release recovery is pinned to the source run and tag', () {
    ...
    expect(workflow, contains('test -f artifacts/vokusz-android.apk'));
    expect(workflow, contains('test -f artifacts/vokusz-web.zip'));
    expect(
      workflow,
      contains("test ! -e artifacts/vokusz-macos-universal.dmg"),
    );
    expect(
      workflow,
      contains("test ! -e artifacts/vokusz-windows-x86_64-setup.exe"),
    );
  ...
  test(
    ...
    () async {
      ...
      final temp = await Directory.systemTemp.createTemp('vokusz-apksigner-');
