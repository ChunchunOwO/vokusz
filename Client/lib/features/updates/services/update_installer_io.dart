...
class UpdateInstaller {
  ...
  static const _androidChannel = MethodChannel('com.vokusz.app/installer');
  ...
  static bool _probeInstallRootWritable() {
    try {
      final probe = File(
        p.join(p.dirname(_installRoot), '.vokusz-update-probe'),
      );
  ...
  Future<String> download(
    ...
    final staging = Directory(p.join(dir.path, 'vokusz-update'));
    ...
    final req = http.Request('GET', Uri.parse(url))
      ..headers['User-Agent'] = 'vokusz-updater'
  ...
  static String get _installRoot {
    ...
    if (UniversalPlatform.isMacOS) {
      // …/Vokusz.app/Contents/MacOS/vokusz → …/Vokusz.app
  ...
  Future<File> _writeHelper(String name, String contents) async {
    final dir = await _temporaryDirectory();
    final helper = File(p.join(dir.path, 'vokusz-update', name));
  ...
  Future<void> _installWindows(File zip) async {
    ...
    final staged = await _extract(
      zip,
      Directory(p.join(dir.path, 'vokusz-update', 'staged')),
    );
...
if mv "$dst" "$dst.old" 2>/dev/null; then
  if mv "${staged.path}" "$dst" 2>/dev/null && [ -f "$dst/vokusz" ]; then
    chmod +x "$dst/vokusz" 2>/dev/null
    rm -rf "$dst.old"
  else
    rm -rf "$dst"; mv "$dst.old" "$dst"
  fi
fi
nohup "$dst/vokusz" >/dev/null 2>&1 &
rm -f "\$0"
''');
...
''');
  ...
  Future<void> _installMacOs(File dmg) async {
    ...
    final srcApp = p.join(mountPoint, 'Vokusz.app');
    if (!Directory(srcApp).existsSync()) {
      await Process.run('hdiutil', ['detach', mountPoint, '-force']);
      throw UpdateInstallException('The update image is missing Vokusz.app.');
    }
    final dst = _installRoot; // …/Vokusz.app
    final pid = _pid;
    final sh = await _writeHelper('apply.sh', '''
#!/usr/bin/env bash
set -u
