...
`applinks:www.vokusz.app`; regenerate `IOS_PROFILE_BASE64` after enabling the
...
- `https://www.vokusz.app/.well-known/apple-app-site-association` is public,
...
The custom `vokusz://` scheme remains accepted. Parser tests cover the HTTPS
...
3. App record + bundle ID `com.vokusz.app` must already exist (they do —
...
2. **Create a service account** in Google Cloud (IAM → Service Accounts) for the
   ...
   ```
3. The Play app record + package `com.vokusz.app` must already
...
To turn macOS on, in App Store Connect → Vokusz → macOS App:
...
uploads `Vokusz-iOS.ipa` as a run artifact, so what you install is what Apple
...
- **A real-device cloud** (BrowserStack App Live, Sauce Labs, AWS Device Farm).
  ...
  provider, and the installed Vokusz build in the review reply. If the session
...
(*"Vokusz has a zero-tolerance policy for objectionable content and abusive
...
Then sign in. A freshly installed app opens its **"Welcome to Vokusz"** tour
...
> [real-device-cloud provider], using Vokusz [version] ([build]). The pointer
...
4. **Nothing may be half-visible.** No frame edge may saw through text or an
   ...
   2.2](https://github.com/ChunchunOwO/vokusz/issues/292) rejects for.
...
`chat.vokusz.app` and could not ship what was there: `#general` read *"just
trying to get vokusz on the ios store"* and *"verity die"*, the roster said
...
```text
Vokusz is a full chat client for Accord servers — self-hosted communities that
...
There is no Vokusz-operated account system. The app connects to a server the
...
21. Anyone can run their own Accord server and connect this app to it — that is
    ...
    Server software: https://github.com/ChunchunOwO/vokusz
```
