/// Launches real Vokusz app processes and drives them through the MCP server
...
class AppInstance {
  ...
  static Future<AppInstance> launch({
    ...
    if (!await instance._waitForMcp(const Duration(seconds: 150))) {
      ...
      final dataDir = Directory('$documents/vokusz/data');
  ...
  /// The app bundle to launch: `VOKUSZ_APP_BIN`, else a local release build.
  ///
  ...
  static String resolveBinary() {
    final explicit = Platform.environment['VOKUSZ_APP_BIN'];
    if (explicit != null && explicit.trim().isNotEmpty) {
      if (File(explicit).existsSync()) return explicit;
      throw AppUnavailable('VOKUSZ_APP_BIN=$explicit does not exist.');
    }
    final release = File('build/linux/x64/release/bundle/vokusz').absolute;
    final debug = File('build/linux/x64/debug/bundle/vokusz').absolute;
    switch (chooseBundle(
      ...
      case BundleChoice.none:
        throw AppUnavailable(
          'No Vokusz app bundle found. Build one first:\n'
          '  flutter build linux --release\n'
          'or point VOKUSZ_APP_BIN at an existing bundle.',
        );
    }
  }
  ...
  static Future<void> seedProfile({
    ...
    final dataDir = Directory('${documentsDirFor(home)}/vokusz/data')
