...
Use the desktop app when you want to run a server for yourself, friends, or a small community on a computer you already own. It is perfect for trying vokusz out or for a home or LAN community. If you need a server that is online around the clock and reachable from anywhere, see [Deploying a Server](deploying-a-server.md) instead — and read the [Self-Hosting Overview](overview.md) for a side-by-side comparison.

## Install

Download the installer for your platform from the [accordserver releases page](https://github.com/ChunchunOwO/vokusz/releases/latest).
...
## Connecting the vokusz client

With Accord running, open the vokusz client, click the **+** button in the sidebar, and add a server.
...
|----------|-------------|
| macOS | `~/Library/Application Support/app.vokusz.Vokusz/` |
| Linux | `$XDG_DATA_HOME/vokusz/` (usually `~/.local/share/vokusz/`) |
| Windows | `%APPDATA%\vokusz\Vokusz\` |
...
When installed through a supported package-manager recipe, Vokusz leaves updates
