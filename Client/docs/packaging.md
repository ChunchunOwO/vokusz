...
Vokusz has review templates for WinGet, Scoop, Chocolatey, Homebrew Cask, and
...
gh api "repos/ChunchunOwO/vokusz/releases/tags/$release_tag" > release-metadata/release.json
gh release download "$release_tag" --repo ChunchunOwO/vokusz \
...
| Scoop | A post-install hook writes `vokusz.package-manager` beside `vokusz.exe`. |
| Flatpak | The launcher exports `VOKUSZ_PACKAGE_MANAGER=flatpak`; runtime detection also recognizes its app ID. |
| Homebrew | A separately signed and notarized DMG built with `--dart-define=PACKAGE_MANAGER=true`. |

Do not modify `Vokusz.app` after signing to add a marker. The cask has no bundle
mutation hook. When the managed DMG exists, regenerate with
`--macos-asset vokusz-macos-universal-package-manager.dmg`; it must appear in the
...
User data lives in `Documents/vokusz/data`. Scoop does not declare `persist`,
...
version with the manifest. The current Inno identity is `Vokusz`, publisher
`vokusz`, AppId `{B8F3A2D1-7C4E-4A9B-8D5F-1E6C3B2A0F47}`. Submit to
...
**Scoop:** validate `scoop/vokusz.json` with Scoop's manifest tests and install
it locally using `scoop install ./dist/packaging/generated/scoop/vokusz.json`.
The release ZIP has `vokusz.exe` at its root. Verify shortcuts, shim, upgrade,
...
artifact is `Vokusz.app`, bundle ID `com.vokusz.app`; the cask declares the
...
**Flatpak:** the proposed app ID is `io.github.ChunchunOwO.vokusz`, matching
...
flatpak-builder --user --install-deps-from=flathub --force-clean \
  build-flatpak dist/packaging/generated/flatpak/io.github.ChunchunOwO.vokusz.json
flatpak run --command=flatpak-builder-lint org.flatpak.Builder manifest \
  dist/packaging/generated/flatpak/io.github.ChunchunOwO.vokusz.json
flatpak run --command=flatpak-builder-lint org.flatpak.Builder appstream \
  dist/packaging/generated/flatpak/io.github.ChunchunOwO.vokusz.metainfo.xml
```
...
voice/video/credential features. The filesystem grant is limited to Vokusz's
...
The desktop entry registers `vokusz://` links.
