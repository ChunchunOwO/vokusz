#!/usr/bin/env bash
#
# Generate Apple signing certificates + provisioning profiles for Vokusz's App
...
#   - The app record + bundle id (com.vokusz.app) already exist.
...
BUNDLE_ID="${APP_BUNDLE_ID:-com.vokusz.app}"
REPO="${GH_REPO:-ChunchunOwO/vokusz}"

IOS_PROFILE_NAME="Vokusz iOS App Store"
MAC_PROFILE_NAME="Vokusz Mac App Store"
...
if ! security cms -D -i "$WORK/ios_profile.profile" 2>/dev/null \
  ...
  echo "error: generated iOS profile does not authorize applinks:www.vokusz.app" >&2
