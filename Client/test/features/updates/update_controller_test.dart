...
/// vokusz-linux-x86_64.deb" failure (#99).
///
...
final _fullReleaseAssets = [
  ...
  _asset('vokusz-android.apk'),
  _asset('vokusz-linux-x86_64.deb'),
  _asset('vokusz-linux-x86_64.tgz'),
  _asset('vokusz-macos-universal.dmg'),
  _asset('vokusz-web.zip'),
  _asset('vokusz-windows-x86_64-setup.exe'),
  _asset('vokusz-windows-x86_64.zip'),
];
...
void main() {
  ...
  group('canInstallInPlace', () {
    ...
    test('legacy .tar.gz bundles stay applicable on a writable install', () {
      ...
      setLatest(c, [
        _asset('vokusz-linux-x86_64.deb'),
        _asset('vokusz-linux-x86_64.tar.gz'),
      ]);
      expect(controllerOf(c).canInstallInPlace, isTrue);
  ...
  group('UpdateState.updateReady', () {
    test('is false when phase is idle even if stagedArchivePath is set', () {
      const state = UpdateState(
        ...
        stagedArchivePath: '/tmp/vokusz.tar.gz',
      );
  ...
  group('platformAsset selection (Unsupported archive regression)', () {
    ...
    test('selects the extension the host installer can actually apply', () {
      ...
      if (ext == null) {
        ...
        // Guard against the Windows web-bundle ambiguity: vokusz-web.zip and
        // vokusz-windows-x86_64.zip both end with .zip; only the latter is a
    ...
    test('prefers windows zip over web zip when web zip is listed first', () {
      // Regression guard for the vokusz-web.zip ambiguity: GitHub returns assets
      // in alphabetical order, so `vokusz-web.zip` sorts before
      // `vokusz-windows-x86_64.zip`. The in-place installer must never pick the
      ...
      final asset = controllerOf(c).platformAsset();
      expect(asset, isNotNull);
      expect(asset!.name, equals('vokusz-windows-x86_64.zip'));
      expect(asset.name, isNot(equals('vokusz-web.zip')));
    });
    ...
    test('a package-only release has no in-place asset but is downloadable', () {
      ...
      if (!UniversalPlatform.isLinux) return;
      final c = makeContainer();
      final deb = _asset('vokusz-linux-x86_64.deb');
      setLatest(c, [deb]);
    ...
    test('platformAssetUrl prefers .tar.gz over .deb for manual download', () {
      if (!UniversalPlatform.isLinux) return;
      final c = makeContainer();
      final tar = _asset('vokusz-linux-x86_64.tar.gz');
      setLatest(c, [_asset('vokusz-linux-x86_64.deb'), tar]);
    ...
    test('canInstallInPlace is true only when an extractable asset exists', () {
      ...
      if (UniversalPlatform.isLinux) {
        setLatest(c, [_asset('vokusz-linux-x86_64.deb')]);
        expect(controllerOf(c).canInstallInPlace, isFalse);
      }
    });
  });
