...
description: Connect to a vokusz server to start chatting.
order: 2
...
To use vokusz, you need to connect to at least one server. A server is a community hosted by someone running accordserver.
...
If you received a link starting with `vokusz://`, opening it in an installed
...
Automatic `vokusz://` registration is included in the Android, iOS, and macOS
...
Discovery, pasted server/invite URLs and `vokusz://connect` or
`vokusz://invite` links first look for a saved account on the target server,
including accounts whose background connection has not started yet. Vokusz
