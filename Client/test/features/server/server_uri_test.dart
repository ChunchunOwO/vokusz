...
void main() {
  group('ServerUri.parseServerUrl', () {
    ...
    test('a vokusz:// string delegates to parseDeepLink', () {
      final parsed = ServerUri.parseServerUrl(
        'vokusz://connect/chat.example.com',
      );
      expect(parsed, isNotNull);
      expect(parsed!.route, 'connect');
      expect(parsed.server?.baseUrl, 'https://chat.example.com');
  ...
  group('ServerUri.parseDeepLink — connect', () {
    test('full connect link with host:port, slug, token and invite', () {
      final parsed = ServerUri.parseDeepLink(
        'vokusz://connect/localhost:3000/my-space?token=t1&invite=i1',
      );
      expect(parsed, isNotNull);
      expect(parsed!.route, 'connect');
    ...
    test('connect link with blank token/invite normalises to null', () {
      final parsed = ServerUri.parseDeepLink(
        'vokusz://connect/chat.example.com?token=&invite=',
      );
      expect(parsed, isNotNull);
      expect(parsed!.token, isNull);
      expect(parsed.invite, isNull);
    ...
    test('connect link decodes its space and channel destination', () {
      final parsed = ServerUri.parseDeepLink(
        'vokusz://connect/chat.example.com/my%20space?channel=release%20notes',
      );
      expect(parsed, isNotNull);
      expect(parsed!.spaceName, 'my space');
  ...
  group('ServerUri.parseDeepLink — invite', () {
    test('invite/<code>@<host> preserves the code and sets route=invite', () {
      final parsed = ServerUri.parseDeepLink(
        'vokusz://invite/ABC123@chat.example.com',
      );
      expect(parsed, isNotNull);
    ...
    test('a non-alphanumeric invite code returns null', () {
      expect(
        ServerUri.parseDeepLink('vokusz://invite/AB-12@chat.example.com'),
        isNull,
      );
    });

    test('an invite link missing the @host returns null', () {
      expect(ServerUri.parseDeepLink('vokusz://invite/ABC123'), isNull);
    });

    test('an invite link with an empty code returns null', () {
      expect(
        ServerUri.parseDeepLink('vokusz://invite/@chat.example.com'),
        isNull,
      );
    });
  });
  ...
  group('ServerUri.parseDeepLink — navigate', () {
    test('navigate/<space>/<channel>?msg=<id> splits all three ids', () {
      final parsed = ServerUri.parseDeepLink(
        'vokusz://navigate/space1/chan1?msg=msg1',
      );
      expect(parsed, isNotNull);
    ...
    test('bare navigate/<space> leaves channel and message null', () {
      final parsed = ServerUri.parseDeepLink('vokusz://navigate/space1');
      expect(parsed, isNotNull);
      expect(parsed!.spaceId, 'space1');
    ...
    test('message destination without a channel is rejected', () {
      expect(
        ServerUri.parseDeepLink('vokusz://navigate/space1?msg=msg1'),
        isNull,
      );
    });
    ...
    test('extra destination path segments are rejected', () {
      expect(
        ServerUri.parseDeepLink('vokusz://navigate/space1/chan1/extra'),
        isNull,
      );
    });
    ...
    test(
      ...
      () {
        expect(
          ServerUri.parseDeepLink('vokusz://navigate/space1/channel1?msg=%ZZ'),
          isNull,
        );
        expect(
          ServerUri.parseDeepLink(
            'vokusz://connect/chat.example/team?channel=%ZZ',
          ),
          isNull,
        );
      },
  ...
  group('ServerUri.parseDeepLink — malformed', () {
    test('a non-vokusz scheme returns null', () {
      expect(ServerUri.parseDeepLink('https://chat.example.com'), isNull);
    });

    test('an unknown route returns null', () {
      expect(ServerUri.parseDeepLink('vokusz://teleport/space1'), isNull);
    });

    test('an empty payload returns null', () {
      expect(ServerUri.parseDeepLink('vokusz://connect/'), isNull);
    });
    ...
    test('the scheme with no body returns null', () {
      expect(ServerUri.parseDeepLink('vokusz://'), isNull);
    });
  });
  ...
  group('ServerUri host validation', () {
    ...
    test('hosts containing < > ; \' or " are rejected', () {
      for (final bad in ['ho<st', 'ho>st', 'ho;st', "ho'st", 'ho"st']) {
        expect(
          ServerUri.parseDeepLink('vokusz://connect/$bad'),
          isNull,
    ...
    test('an invite link with an invalid host is rejected', () {
      expect(
        ServerUri.parseDeepLink('vokusz://invite/ABC123@bad host'),
        isNull,
      );
    });

    test('a clean host:port authority validates and is accepted', () {
      final parsed = ServerUri.parseDeepLink('vokusz://connect/good.host:443');
      expect(parsed, isNotNull);
