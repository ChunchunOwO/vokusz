...
- pasting a link into Slack, iMessage, X, Facebook, or other chat apps produces a
  rich social card.
...
- **Humans** always get a lightweight redirect landing page that tries the
  `vokusz://connect/{host}/{slug}` deep link, then falls back to the web
...
```json
{
  "version": "1.0",
  "type": "link",
  "title": "How I self-host vokusz on a Pi",
  "author_name": "Alice",
...
<link rel="alternate" type="application/json+oembed"
      href="https://your.server/oembed?url=...&format=json" title="vokusz oEmbed">
```
...
- **Share with those who have the app** → `vokusz://navigate/{spaceId}/{channelId}?msg={postId}`
- **Share with the internet** → `https://{host}/s/{slug}/{channel}/{postId}`

The first opens the post directly in a vokusz client; the second is the public,
